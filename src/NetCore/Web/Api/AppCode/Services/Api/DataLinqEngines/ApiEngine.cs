...
public class ApiEngine : IDataLinqSelectEngine, IDataLinqEngineCache
{
    ...
    async public Task<(object[] records, bool isOrdered)> SelectAsync(DataLinqEndPoint endPoint, DataLinqEndPointQuery query, NameValueCollection arguments)
    {
        ...
        foreach (var parameterName in arguments.AllKeys)
        {
            ...
            if (parameterName.ToLower() == "_orderby" && url.Contains("{{" + parameterName + "}}"))
            {
                isOrdered = true;
                List<string> orderItems = new List<string>();
                foreach (string orderField in arguments[parameterName].Split(','))
                {
                ...
                url = url.Replace("{{" + parameterName + "}}", String.Join(",", orderItems));
            }
        ...
        if (featureCollection.Features != null)
        {
            foreach (var feature in featureCollection.Features)
            {
                ...
                //if (feature.Bounds != null && feature.Bounds.Length == 4)
                //{
                //    expandoDict.Add("_bbox", new RecordBoundingBox()
                //    {
                //        SouthWest = new RecordLocation()
                //        {
                //            Longitude = feature.Bounds[0],
                //            Latitude = feature.Bounds[1]
                //        },
                //        NorthEast = new RecordLocation()
                //        {
                //            Longitude = feature.Bounds[2],
                //            Latitude = feature.Bounds[3]
                //        }
                //    });
                //}
                foreach (var key in feature.Properties.Keys)
                {
                    ...
                    expandoDict.Add(key, JSerializer.AsValueIfJsonValueType(feature.Properties[key]));
                }
                result.Add(expando);
            }
        }
