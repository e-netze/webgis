webgis.map._graphics = function (map) {
    ...
    this._sketch.events.on(['ongetsvalid'], function (e, sender) {
        if (this._serializing === false && this._defaultBehaviour(this._tool)) {
            if (this._tool === 'text') {
                this._sketchMetaText = this._sketch.getText();
            }
    ...
    this.setTool = function (tool) {
        ...
        if (this._sketch.isValid()) {
            this.assumeCurrentElement(this._sketch && this._sketch.isValid());
        }
    ...
    this.getToolDescription = function (tool) {
        return webgis.i18n.get("redlining-" + (tool || this._tool)) || '';
    };
    ...
    this._onElementClick = function (e) {
        ...
        if (this.map.graphics._isGraphicsToolActive()) {  // Nur mit aktivem Redling Werkzeug sollte das selektieren funktionieren.
            ...
            if (this.type === 'symbol') {
                ...
                var element = this.map.graphics._elementByFrameworkElement(this.frameworkElement);
                if (element) {
    ...
    this._refreshInfoContainers = function () {
        ...
        if ($.fn.webgis_graphicsInfoContainer) {
            ...
            if ($().webgis_graphicsInfoContainer('hasContainers', containerOptions)) {
                $(null).webgis_graphicsInfoContainer('refreshAllContainers', containerOptions);
                ...
                // create simple readonly container
                var map = this._map;
    ...
    this.assumeCurrentElement = function (silent) {
        //console.log('assumeCurrentElement-valid-sketch', this._sketch.isValid());
        if (!this._sketch || !this._sketch.isValid())
            return;

        if (this._defaultBehaviour(this._tool)) {
            var elements = this._sketch.cloneGraphicElements();
            for (var e in elements) {
                var element = elements[e]; //this._sketch.cloneGraphicElement();
                element.addTo(this._map.frameworkElement);
                var symbolElement = {
                    type: this._tool,
                    map: this._map,
    ...
    this.labelElement = function (element) {
        ...
        switch (element.type) {
            ...
            case 'point':
                ...
                label = element.metaText || webgis.i18n.get("redlining-tool-point");
                break;
        }
    ...
    this.fromGeoJson = function (gr) {
        try {
            ...
            for (var i in gr.geojson.features) {
                ...
                this.assumeCurrentElement(true);
            }
    ...
    this.getElements = function () {
        ...
        if (this._tool === 'symbol') {
            currentElement = this._elementByFrameworkElement(this._currentMarker);
            if (currentElement !== null) {
                result.push({
                    isCurrent: true,
                    type: currentElement.type,
                    originalElement: currentElement,
                    metaText: currentElement.metaText
                });
            }
        } else if (this._tool === 'point') {
            currentElement = this._elementByFrameworkElement(this._currentPointMarker);
            if (currentElement !== null) {
                result.push({
                    isCurrent: true,
                    type: currentElement.type,
        ...
        } else if (this._defaultBehaviour(this._tool)) {
            if (this._sketch.isValid()) {
                result.push({
                    isCurrent: true,
                    type: this._tool,
        ...
        for (var i in this._elements) {
            var element = this._elements[i];
            if (!element.frameworkElement || element === currentElement)
                continue;
