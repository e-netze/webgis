...
(function (UI) {
    ...
    UI.PropertyGridItem = PropertyGridItem;
    var PropertyGridOptions = /** @class */ (function () {
        function PropertyGridOptions() {
        }
        return PropertyGridOptions;
    }());
    UI.PropertyGridOptions = PropertyGridOptions;
    var PropertyGrid = /** @class */ (function () {
        function PropertyGrid(element, options, method) {
        ...
        PropertyGrid.prototype.OnCreate = function () {
            ...
            for (var p in this.options.properties) {
                ...
                var $propWrapper = $("<div>").addClass('propertygrid-property-wrapper').appendTo($category.find('.propertygrid-category-body'));
                var $label = $("<div><div>")
                    .text(property.displayName)
                    .addClass('propertygrid-label')
                    .appendTo($propWrapper);
                var $value = $("<div>").addClass('propertygrid-value').appendTo($propWrapper);
                ...
                if (property.hasEditor === true) {
                    var $editor = $("<div>...</div>")
                        ...
                        CMS.api('NodePropertyEditor', { path: $propertyGrid.attr('data-path'), property: property, data: JSON.stringify(data) }, function (result) {
                            $('body').modalDialog({
                                ...
                                onCommit: function ($sender, $modal) {
                                    ...
                                    CMS.api('NodePropertyEditorCommit', { path: $propertyGrid.attr('data-path'), property: property, data: JSON.stringify(data) }, function (result) {
                    ...
                    var $editor = $("<div>...</div>")
                        ...
                        CMS.api('NodeProperties', { path: $propertyGrid.attr('data-path'), subProperty: subProperty, data: JSON.stringify(data) }, function (result) {
                            $('body').modalDialog({
                                title: result.displayName,
                                onLoad: function ($content) {
                                    $content.propertyGrid({ properties: result.properties, path: result.path, subProperty: result.subProperty });
                                },
                                onCommit: function ($sender, $modal) {
                                    $sender.addClass('loading');
                                    var $grid = $modal.find('.cms-propertygrid');
                                    var data = CMS.propertyGridData($grid);
                                    CMS.api('NodePropertiesCommit', { path: $propertyGrid.attr('data-path'), subProperty: subProperty, data: JSON.stringify(data) }, function (result) {
                                        $sender.removeClass('loading');
                                        if (typeof result.value !== 'undefined') {
                                            var $target = $propertyGrid.find("input[name='" + $editorButton.attr('data-property') + "'], select[name='" + $editorButton.attr('data-property') + "']");
                                            $target.val(result.value);
        ...
        PropertyGrid.prototype.RefreshData = function () {
    ...
    //  TypeEditor Array
    //
    var TypeEditorArray = /** @class */ (function () {
        function TypeEditorArray(control, displayname, arr) {
            this.control = control;
            var editor = this;
            CMS.showModal(displayname, function ($content, $modal) {
                ...
                var $input = $("<input type='text' placeholder='neuer Wert...'>")
                    ...
                    .appendTo($content);
                var inchMeter = 0.0254;
                // empiric
                // ???
                var dpi = 95.999807999999988673558652928; // inchMeter * 1000.0 / 0.2644583862501058;   // wmts 0.28mm -> 1 Pixel;
                var dpm = dpi / inchMeter;
                ...
                var $ul = $("<ul>").addClass('sortable-list').appendTo($content);
                for (var i = 0; i < arr.length; i++) {
                    ...
                    var $li = $("<li class='array-element'></li>")
                        .attr('title', title)
                        .appendTo($ul);
                ...
                $modal.find('li.array-element').each(function (i, e) {
                    var val = $(e).children('input').val(); //$(e).data('value');
                    try {
                ...
                $modal.data('editor').control.data('val', array).val(JSON.stringify(array));
                CMS.closeModal($modal);
            }, null);
        }
        return TypeEditorArray;
    }());
})(UI || (UI = {}));
