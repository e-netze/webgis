...
public class Clipper : ClipperBase
{
  ...
  public ZFillCallback ZFillFunction { get; set; }
    ...
    {
        return Execute(clipType, solution, FillType, FillType, cts);
    }
    //------------------------------------------------------------------------------
    ...
    public bool Execute(ClipType clipType, PolyTree polytree,
        ...
        PolyFillType FillType = PolyFillType.pftEvenOdd)
    {
        return Execute(clipType, polytree, FillType, FillType, cts);
    }
    //------------------------------------------------------------------------------
    ...
    private bool ExecuteInternal(CancellationTokenSource cts)
    {
        try
        {
            Reset();
  ...
  //------------------------------------------------------------------------------
          ...
          PolyFillType fillType = PolyFillType.pftEvenOdd)
    {
        ...
        c.Execute(ClipType.ctUnion, result, fillType, fillType, cts);
        return result;
    }
    //------------------------------------------------------------------------------
    ...
    public static Paths MinkowskiSum(Path pattern, Path path, bool pathIsClosed, CancellationTokenSource cts)
    {
        Paths paths = Minkowski(pattern, path, true, pathIsClosed);
        Clipper c = new Clipper();
        c.AddPaths(paths, PolyType.ptSubject, true);
        c.Execute(ClipType.ctUnion, paths, PolyFillType.pftNonZero, PolyFillType.pftNonZero, cts);
        return paths;
    }
    //------------------------------------------------------------------------------
    ...
    public static Paths MinkowskiSum(Path pattern, Paths paths, bool pathIsClosed, CancellationTokenSource cts)
    {
        ...
        c.Execute(ClipType.ctUnion, solution,
          PolyFillType.pftNonZero, PolyFillType.pftNonZero, cts);
        return solution;
    }
    //------------------------------------------------------------------------------

    public static Paths MinkowskiDiff(Path poly1, Path poly2, CancellationTokenSource cts)
    {
...
public class ClipperOffset
{
    ...
    public void AddPaths(Paths paths, JoinType joinType, EndType endType, CancellationTokenSource cts)
    {
    ...
    public void Execute(ref Paths solution, double delta, CancellationTokenSource cts)
    {
        solution.Clear();
        FixOrientations();
        DoOffset(delta);
        ...
        if (delta > 0)
        {
            clpr.Execute(ClipType.ctUnion, solution,
              PolyFillType.pftPositive, PolyFillType.pftPositive, cts);
        }
        else
        {
            ...
            clpr.Execute(ClipType.ctUnion, solution, PolyFillType.pftNegative, PolyFillType.pftNegative, cts);
            if (solution.Count > 0)
            {
                solution.RemoveAt(0);
            }
    ...
    public void Execute(ref PolyTree solution, double delta, CancellationTokenSource cts)
    {
        solution.Clear();
        FixOrientations();
        DoOffset(delta);
