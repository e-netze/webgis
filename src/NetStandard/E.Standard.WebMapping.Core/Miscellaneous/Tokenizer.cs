...
public class Tokenizer
{
    ...
    public delegate void TransformTokenEventHandler(ref string text, int gmlVersion);
    public event TransformTokenEventHandler OnTransformToken = null;
    public string Transform(int gmlVersion)
    {
        return AddStrings(TransformToken(_parentToken, gmlVersion));
    }
    public string TransformXML(int gmlVersion)
    {
        return AddStringsXML(TransformToken(_parentToken, gmlVersion));
    }
    private string TransformToken(Token token, int gmlVersion)
    {
        ...
        {
            OnTransformToken(ref text, gmlVersion);
        }

        for (int i = 0; i < token.ChildTokens.Count; i++)
        {
            text = text.Replace("[" + i + "]", TransformToken(token.ChildTokens[i], gmlVersion));
        }
