...
public class TableStorage
{
    ...
    async public Task<T[]> AllTableEntitiesSince<T>(string tableName, string partitionKey, DateTimeOffset since)
        where T : ITableEntity, new()
    {
        CloudStorageAccount storageAccount = CloudStorageAccount.Parse(_connectionString);
        ...
        TableQuery<T> query = new TableQuery<T>().Where(dateRangeFilter);
    ...
    async public Task<T[]> AllTableEntitiesTo<T>(string tableName, string partitionKey, string rowKey)
        where T : ITableEntity, new()
    {
        CloudStorageAccount storageAccount = CloudStorageAccount.Parse(_connectionString);
