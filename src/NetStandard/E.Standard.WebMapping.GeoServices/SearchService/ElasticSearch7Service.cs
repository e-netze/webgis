...
public class ElasticSearch7Service : ISearchService3
{
    ...
    public ElasticSearch7Service(CmsNode node)
    {
        this.Id = node.Url;
        this.Name = node.Name;
    ...
    async public Task<SearchTypeMetadata> GetTypeMetadataAsync(IHttpService httpService, string metaId)
    {
        try
        {
            string url = _serviceUrl + ".meta/_doc/" + metaId;
    ...
    async public Task<IEnumerable<SearchTypeMetadata>> GetTypesMetadataAsync(IHttpService httpService)
    {
        ...
        {
            Console.WriteLine($"GetTypesMetadataAsync: {url}");
    ...
    async public Task<SearchServiceItems> Query3Async(IHttpService httpService, string term, int rows, IEnumerable<string> categories, Envelope queryBBox, int targetProjId = 4326)
    {
        ...
        string url = $"{_serviceUrl}/_search?size={(rows > 0 ? rows : _rows)}", queryJson = null;

        if (queryBBox == null)
        {
            url = $"{url}&q={analizedTerm}".Replace("{term}", analizedTerm).Replace("[term]", analizedTerm);
        }
        else
        {
            queryJson = ToQueryJson(analizedTerm, queryBBox);
        }

        ElasticSearchResponse response = null;
        try
        {
            string json = String.IsNullOrEmpty(queryJson) ?
                await GetAsync(httpService, url) :
                await PostJsonAsync(httpService, url, queryJson);
            response = JsonConvert.DeserializeObject<ElasticSearchResponse>(json);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Query3Async: {url}");
    ...
    public string ToQueryJson(string analizedTerm, Envelope bbox)
    {
        return $@"{{
  ""query"": {{
    ""bool"": {{
      ""must"": {{
                ""query_string"": {{
                    ""query"":{JsonConvert.ToString(analizedTerm)}
        }}
            }},
      ""filter"": {{
                ""geo_bounding_box"": {{
                    ""geo"": {{
                        ""top_left"": ""{bbox.MaxY.ToPlatformNumberString()}, {bbox.MinX.ToPlatformNumberString()}"",
                        ""bottom_right"": ""{bbox.MinY.ToPlatformNumberString()}, {bbox.MaxX.ToPlatformNumberString()}""
                    }}
                }}
            }}
        }}
    ...
    async private Task<string> GetAsync(IHttpService httpService, string url)
    {
        return await httpService.GetStringAsync(url, encoding: Encoding.UTF8);
        //using (var httpResponse = await (_useProxy ? _httpClientWithProxy : _httpClient).GetAsync(url))
        //{
        //    return await httpResponse.Content.ReadAsStringAsync();
        //}
    }

    async private Task<string> PostJsonAsync(IHttpService httpService, string url, string json)
    {
        return await httpService.PostJsonAsync(url, json);
        //using (var data = new StringContent(json, Encoding.UTF8, "application/json"))
        //using (var httpResponse = await (_useProxy ? _httpClientWithProxy : _httpClient).PostAsync(url, data))
        //{
    ...
    private string AnalyseTerm(string term, IEnumerable<string> categories)
    {
        ...
        sb.Append($"({term}) OR ({term}*)");

        if (singleTerms.Count > 1)
        {
    ...
    public class Hits
    {
        ...
        public ValueClass Total { get; set; }
    ...
    {
        [JsonProperty("valule")]
        [System.Text.Json.Serialization.JsonPropertyName("valule")]
