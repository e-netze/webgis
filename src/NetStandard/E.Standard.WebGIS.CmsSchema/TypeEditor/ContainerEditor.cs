...
public class ContainerEditor : UserControl, IUITypeEditor
{
    ...
    ListBox list = new ListBox("list") { Label = "Darstellungsvarianten Container", MultiSelect = false, SelectAndCommit = true };
    ...
    public IUIControl GetUIControl(ITypeEditorContext context)
    {
        ...
        ItemOrder itemOrder = new ItemOrder(cms.ConnectionString + @"/etc/containers");
        foreach (string item in itemOrder.Items)
        {
            var fi = DocumentFactory.DocumentInfo(cms.ConnectionString + @"/etc/containers/" + item);
