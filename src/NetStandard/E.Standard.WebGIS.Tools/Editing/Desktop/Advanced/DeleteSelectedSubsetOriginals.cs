...
public class DeleteSelectedSubsetOriginals : IApiServerToolAsync, IApiChildTool
{
    #region IApiButton

    public string Name => "Original Features Löschen";
    ...
    async public Task<ApiEventResponse> OnButtonClick(IBridge bridge, ApiToolEventArguments e)
    {
        ...
        var subset = e.GetArray<int>(EditEnvironment.EditTheme.EditFeatureIdsSubsetId);

        var selectedFeatures = subset == null || subset.Length == 0 ?
            null :
            await e.FeaturesFromSelectionAsync(bridge, objectIdsSubset: subset);
            ...
            new UIButtonGroup()
            {
                elements = new IUIElement[]
                {
                    new UIButton(UIButton.UIButtonType.clientbutton, ApiClientButtonCommand.setparenttool )
                    {
                        ...
                        css = UICss.ToClass(new string[] { UICss.CancelButtonStyle, UICss.OptionButtonStyle, UICss.WrapText })
                    },
                    new UIButton(UIButton.UIButtonType.servertoolcommand, "delete-original")
                    {
                        ...
                        css = UICss.ToClass(new string[] { UICss.DangerButtonStyle, UICss.OptionButtonStyle, UICss.WrapText })
                    }
                }
            },
    ...
    async public Task<ApiEventResponse> OnDeleteOriginal(IBridge bridge, ApiToolEventArguments e)
    {
        ...
        {
            throw new Exception("Unknown error: no ids subset commited");
        }

        var features = subset == null || subset.Length == 0 ?
            null :
            await e.FeaturesFromSelectionAsync(bridge, objectIdsSubset: subset);
