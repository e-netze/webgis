...
public class MassAttributation : IApiServerToolAsync, IApiChildTool
{
    ...
    async public Task<ApiEventResponse> OnButtonClick(IBridge bridge, ApiToolEventArguments e)
    {
        ...
        editTheme.CheckEditPermissions(EditEnvironment.EditFeatureCommand.MassAttribution, null);
    ...
    async public Task<ApiEventResponse> OnSave(IBridge bridge, ApiToolEventArguments e)
    {
        ...
        var featureTemplate = editEnvironment.GetFeature(bridge, e, checkApplyField: true);
        if (featureTemplate.Attributes == null || featureTemplate.Attributes.Count == 0)
        {
        ...
        return new ApiEventResponse()
        {
            ...
            ToolUndos = !editEnvironment.HasUndoables ?
                null :
                editEnvironment.Undoables.Select(u => new ToolUndoDTO(u, null)
                {
